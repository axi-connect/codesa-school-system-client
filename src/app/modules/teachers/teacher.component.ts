...
export class TeacherComponent implements OnInit{
  ...
  async ngOnInit(): Promise<void> {
    ...
    try {
      const teachersResponse = await fetch(`${environment.apiUrl}/teachers`, {
        headers: {
      ...
      this.formConfig = {
        ...
        fields: [
          ...
          { label: 'Fecha de Contratación', name: 'fecha_contratacion', type: 'date', required: true},
        ]
      }
    } catch (error) {
      this.users = [];
  ...
  async createTeacher(formData: any): Promise<void> {
    ...
    const token = localStorage.getItem('auth');
    try {
      const response = await fetch(`${environment.apiUrl}/teachers`, {
        method: 'POST',
        headers: {
  ...
  async updateTeacher(formData: any): Promise<void> {
    ...
    try {
      const response = await fetch(`${environment.apiUrl}/teachers/${this.teacherToEdit?.id_user}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  async deleteUser(user: User): Promise<void> {
    ...
    const token = localStorage.getItem('auth');
    try {
      const response = await fetch(`${environment.apiUrl}/teachers/${user.id_user}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
